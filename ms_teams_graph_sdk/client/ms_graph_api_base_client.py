...
class MSGraphAPIBaseClient:
    ...
    def _get_api_url(self, endpoint: str) -> str:
        api_url = f"{settings.MICROSOFT_GRAPH_BASE_API_URL}{settings.MICROSOFT_GRAPH_API_VERSION}/{endpoint}/"
        return api_url
    ...
    def _get_request_params(self, params: Optional[Dict] = None, pagination: bool = True) -> Dict:
        if settings.PAGINATION_COUNT and pagination:
            params = params if params else {}
            if "$top" not in params:
                params["$top"] = settings.PAGINATION_COUNT
        return params
    ...
    def check_connection(self):
        try:
            ...
            self.logger.error(f'Error connecting to the Microsoft Graph API: {e}!')
            return False